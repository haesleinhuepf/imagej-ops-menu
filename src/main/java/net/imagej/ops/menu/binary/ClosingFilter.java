 ...
 */
@Plugin(type = Command.class, menuPath = "Ops>Binary>Closing")
public class ClosingFilter<T extends RealType<T>> implements Command {
    ...
    public void run() {
        ...
        opService.filter().max(temp, input, shape);
        opService.filter().min(output, temp, shape);

        ImagePlus imp = ImageJFunctions.wrap(output, "Binary closing (radius = " + radius + ") of " + currentData.getTitle());
        ...
        command = command + "output = ops.filter().max(temp, input, shape);\n";
        command = command + "output = ops.filter().min(output, temp, shape);\n";
