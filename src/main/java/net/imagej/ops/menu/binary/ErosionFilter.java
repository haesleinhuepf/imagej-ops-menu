 ...
 */
@Plugin(type = Command.class, menuPath = "Ops>Binary>Erosion")
public class ErosionFilter<T extends RealType<T>> implements Command {
    ...
    public void run() {
        ...
        opService.filter().min(output, input, shape);

        ImagePlus imp = ImageJFunctions.wrap(output, "Binary erosion (radius = " + radius + ") of " + currentData.getTitle());
        ...
        command = command + "output = ops.filter().min(output, input, shape);\n";
