...
@Plugin(type = Command.class, menuPath = "Ops>Filtering>Median")
public class MedianFilter<T extends RealType<T>> implements Command {
    ...
    public void run() {
        ...
        opService.filter().median(output, input, shape);
        ...
        command = command + "output = ops.filter().median(output, input, shape);\n";
